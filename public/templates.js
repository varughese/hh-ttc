angular.module('nhs').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/members.html',
    "<h2>Member List</h2><table class=table><thead><tr><th>Name</th><th>Checked</th><th>Total Hours</th></tr></thead><tbody><tr ng-repeat=\"member in members\" ui-sref=\"singleMember({memberID: member._id})\"><td>{{member.firstname + \" \" + member.lastname}}</td><td>{{member.checked}}</td><td>{{member.totalHours}}</td></tr></tbody></table>"
  );


  $templateCache.put('templates/signup.html',
    "<div class=\"row col-md-6 col-md-offset-3 login signup\"><div class=col-sm-12><img class=img-responsive src=../images/nhs-logo.jpeg></div><div class=login-main><h1>Create Account</h1><form ng-submit=signup()><div class=form-group><label>First Name</label><input required class=form-control ng-model=user.firstname></div><div class=form-group><label>Last Name</label><input required class=form-control ng-model=user.lastname></div><div class=form-group><label>Username</label><input required class=form-control ng-model=user.username></div><div class=form-group><label>Password</label><input required class=form-control ng-model=user.password></div><button class=\"btn btn-block btn-primary\" type=submit>Signup</button></form></div></div>"
  );
